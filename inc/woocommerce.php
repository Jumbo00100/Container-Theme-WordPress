...
function ccn_sale_flash() {
	return '<span class="onsale">' . esc_html__( 'Angebot!', 'containertheme' ) . '</span>';
}
...
function ccn_catalog_filter() {
	...
	echo '<nav class="catalog-filter" aria-label="' . esc_attr__( 'Kategorien', 'containertheme' ) . '">';
	printf(
		'<a class="catalog-filter-link%s" href="%s">%s</a>',
		$current ? '' : ' is-active',
		esc_url( get_permalink( wc_get_page_id( 'shop' ) ) ),
		esc_html__( 'Alle', 'containertheme' )
	);
