...
if ( ! class_exists( 'WooCommerce' ) ) {
	if ( ! empty( $is_preview ) ) {
		echo '<p><em>' . esc_html__( 'Featured products: WooCommerce is not active.', 'containertheme' ) . '</em></p>';
	}
	return;
}
...
if ( ! $ccn_q->have_posts() ) {
	wp_reset_postdata();
	if ( ! empty( $is_preview ) ) {
		echo '<p><em>' . esc_html__( 'Featured products: no products matched (mark products as featured or set a category).', 'containertheme' ) . '</em></p>';
	}
	return;
}
?>
