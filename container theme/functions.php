<?php
/**
 * ContainerTheme — setup, asset loading, includes.
 */

add_action( 'after_setup_theme', 'ccn_setup' );
function ccn_setup() {
	load_theme_textdomain( 'containertheme', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'primary' => esc_html__( 'Primary Menu', 'containertheme' ),
			'footer'  => esc_html__( 'Footer Menu', 'containertheme' ),
		)
	);
}
...
function ccn_enqueue_assets() {
	...
	wp_enqueue_style( 'containertheme-', $dir . '/assets/css/theme.css', array(), ccn_asset_version( '/assets/css/theme.css' ) );
...
function ccn_skip_link() {
	echo '<a href="#content" class="skip-link screen-reader-text">' . esc_html__( 'Skip to the content', 'containertheme' ) . '</a>';
}
...
function ccn_read_more_link() {
	...
	return ' <a href="' . esc_url( get_permalink() ) . '" class="more-link">' . sprintf( __( '&hellip;%s', 'containertheme' ), '<span class="screen-reader-text"> ' . esc_html( get_the_title() ) . '</span>' ) . '</a>';
}
