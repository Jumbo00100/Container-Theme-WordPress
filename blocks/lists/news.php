...
if ( ! $ccn_q->have_posts() ) {
	wp_reset_postdata();
	if ( ! empty( $is_preview ) ) {
		echo '<p><em>' . esc_html__( 'News: no posts published yet.', 'containertheme' ) . '</em></p>';
	}
	return;
}
?>
			...
			while ( $ccn_q->have_posts() ) :
				...
				<article class="ccn-news-card">
					...
					<div class="ccn-news-body">
						...
						<a class="ccn-news-more" href="<?php the_permalink(); ?>"><?php esc_html_e( 'Weiterlesen', 'containertheme' ); ?><span class="screen-reader-text"> <?php the_title(); ?></span></a>
					</div>
				</article>
			<?php endwhile; ?>
		</div>
